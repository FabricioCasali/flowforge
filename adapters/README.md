...
- **A sessão viva existe no Claude Code** (pelo hook `Stop` do plugin, que rearma a escuta a cada
  fim de turno; sem o plugin, pela ferramenta Monitor, que expira em no máximo 30 min, e aí o canvas
  avisa e o usuário pede a reconexão) **e no OpenCode** (pelo servidor HTTP da
